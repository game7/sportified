...
namespace :league do
  ...
  task exclude_prefixed_games: :environment do
    Game.where('text_before IS NOT NULL').where("text_before != ''").each do |game|
      game.update(exclude_from_team_records: true)
    end
  end
  ...
  task :recalculate_player_results, [:season] => :environment do |_t, args|
    League::Game.for_season(args[:season]).where('statsheet_id IS NOT NULL').includes(statsheet: %i[goals
                                                                                                    penalties]).each do |game|
      puts '--------------------------'
      puts game.summary
      puts '--------------------------'
  ...
  desc 'retriggers player statistics for specified season'
  task :recalculate_goalie_games_played, [:season] => :environment do |_t, args|
    League::Game.for_season(args[:season]).where('statsheet_id IS NOT NULL').includes(statsheet: %i[goals
                                                                                                    penalties]).each do |game|
      puts '--------------------------'
      puts game.summary
    ...
    Tenant.all.each do |tenant|
      ...
      Team.all.each do |team|
        ...
        team.record.results.each do |result|
          dirty = false
          game = result.game
          puts "  -> Result for game on #{result.played_on} vs. #{result.opponent_name} [#{game.summary}]"
  ...
  task update_statsheet_player_names: :environment do
    Tenant.all.each do |tenant|
      ...
      Game.all.includes(:statsheet).each do |game|
        puts 'NEXT GAME...'
        next unless game.has_statsheet?

        puts '-- HAS STATSHEET'
        game.statsheet.players.includes(:player).each do |p|
          next unless p.player
  ...
  task load_rosters: :environment do
    ...
    players.each do |p|
      ...
      player.email = player.email.downcase if player.email.present?
      if player.save
