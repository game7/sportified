...
class League::Admin::GameUploadsController < League::Admin::BaseLeagueController
  ...
  def create
    ...
    if @game_upload.save
      redirect_to edit_league_admin_game_upload_path(@game_upload.id), :success => "Game Upload has been created."
    else
  ...
  def update
    if @game_upload.update_attributes(params[:game_upload])
      redirect_to league_admin_game_uploads_path, :notice => 'Game Upload was successfully updated.'
    else
      render :action => "edit"    
    end
  end
  ...
  def complete
    unless @game_upload.can_complete?
      ...
      processor.complete!
      redirect_to league_admin_game_uploads_path, :success => "#{processor.games.length} Games have been successfully uploaded"
    end
  ...
  def add_games_breadcrumb
    ...
    add_breadcrumb 'Game Uploads', league_admin_game_uploads_path
  end
