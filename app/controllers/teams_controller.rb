...
class TeamsController < BaseDivisionSeasonController
  ...
  def links_to_division
    links = Division.asc(:name).each.collect{ |d| [d.name + " Division", teams_path(:division_slug => d.slug, :season_slug => params[:season_slug])] }
    links.insert(0, ['All Divisions', teams_path])
  end
