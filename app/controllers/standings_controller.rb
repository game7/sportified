class StandingsController < BaseLeagueController
  ...
  def index
    ...
    @teams = @league.teams.for_season(@season).asc('seed').desc('record.pts').desc('record.w').desc('record.margin')
  end
  
  private
  
  def get_season_options
    @season_options = Season.all.desc(:starts_on).collect{|s| [s.name, standings_path(:league_slug => @league.slug, :season_slug => s == @season ? nil : s.slug)]}
  end  
