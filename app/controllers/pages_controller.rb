...
class PagesController < ApplicationController
  ...
  before_action :load_parent_options, only: %i[new edit]
  before_action :mark_return_point, only: %i[new edit]
  layout 'admin', only: %i[index new create]
  ...
  def show
    ...
    return unless @page.skip_to_first_child and (first_live_child = @page.children.live.order(:position).first).present?
  ...
  def move_up
    ...
    siblings = siblings.insert(current_position - 1, siblings.delete_at(current_position)) if current_position > 0
    puts siblings.to_json
  ...
  def set_child_page_navigation
    ...
    return if @page.root? or children.any?
  ...
  def stripe_connect_error
    ...
    connect.update(status: params[:error])
