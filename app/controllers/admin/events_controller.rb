...
class Admin::EventsController < Admin::AdminController
  skip_before_action :verify_admin, only: %i[index update]
  before_action :verify_admin_or_operations, only: %i[index update]

  before_action :mark_return_point, only: [:destroy]
  before_action :add_events_breadcrumb
  before_action :load_event, only: %i[update destroy]
  ...
  def color_map(events)
    events.collect(&:color_key).uniq.index_with do |b|
      '#' + colorize(b)
    end
  end
