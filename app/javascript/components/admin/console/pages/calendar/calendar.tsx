...
import { Tag, list as listTags } from '../../actions/tags';
import { Event, list as listEvents } from "../../actions/events"
import { withRouter, RouteComponentProps } from "react-router"
import { Modal, Form, Table, Grid, Button, Dropdown, Icon, DropdownItemProps } from 'semantic-ui-react';
import { n } from "../../utils";
import DateTimePicker from '../../components/date-time-picker';
import DateTimeInput from '../../../../../calendar/date-time-input';

function dateToString(date: Date) {
  return moment(date).format('YYYY/MM/DD')
}

export const Calendar: FC<RouteComponentProps<{}>> = ({ history, location }) => {
  ...
  const MonthEvent: FC<{event: Event}> = ({ event }) => {
    ...
    const tagOptions = Object.keys(tags || {}).map(key => ({
      key: tags[key].name,
      text: tags[key].name,
      value: tags[key].name,  
    }))
    ...
    return (
      <Modal trigger={Event} scrolling={false} dimmer="inverted" >
        ...
        <Modal.Content>
          <Form>
            ...
            <Form.Group>
              <DateTimeInput label="Date / Time" value={event.startsOn} />
              <Form.Field>
                <label>Duration</label>
                <input value={event.duration} style={{ width: 80 }}/>
              </Form.Field>
            </Form.Group>
            <Form.Field>
              <label>Tags</label>
            ...
            <Form.Field>
              <Dropdown 
                ...
                allowAdditions
                options={tagOptions}
                value={event.tags.map(id => tags[id].name)}
              />
            </Form.Field>
          </Form>
        </Modal.Content>
  ...
  return (
    <Layout title="Calendar" actions={actions} loading={loading}>
      <React.Fragment>
        <Grid columns={3}>
