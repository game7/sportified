...
function rgbToHex(color: [number, number, number]) {
  return (
    "#" + numberToHex(color[0]) + numberToHex(color[1]) + numberToHex(color[2])
  );
}
        ...
        image.addEventListener(
          ...
          function imageLoaded() {
            const colors = new Set(
              new ColorThief().getPalette(image).map(rgbToHex) as string[]
            );
  ...
  return (
    <React.Fragment>
      ...
      <Card fluid>
        ...
        >
          <SemanticImage
            src={preview || url || "/image.png"}
            size="mini"
            rounded
            floated="left"
            style={{ marginBottom: 0 }}
          />
          <Card.Header style={{ color: colors.secondary }}>
            Team Name
          </Card.Header>
          <Card.Meta style={{ color: colors.accent }}>
            blah, blah, blah
          </Card.Meta>
        </Card.Content>
        <Card.Content extra>
          ...
          <ColorPicker
            ...
            palette={palette}
            onChange={colorChange("secondary")}
          >
            Title
          </ColorPicker>
          <ColorPicker
      ...
      <input
        type="hidden"
        name={`${object}[primary_color]`}
        value={colors.primary}
      />
      <input
        type="hidden"
        name={`${object}[secondary_color]`}
        value={colors.secondary}
      />
      <input
        type="hidden"
        name={`${object}[accent_color]`}
        value={colors.accent}
      />
    </React.Fragment>
  );
