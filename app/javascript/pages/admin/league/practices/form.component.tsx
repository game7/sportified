...
export function LeaguePracticeForm() {
  ...
  const form = useForm<League.Practice>({ initialValues: props.practice });
  const bind = useBind(form);

  function handleSubmit(data: League.Practice) {
    if (practice.id) {
      Inertia.patch(`/next/admin/league/practices/${practice.id}`, {
        practice: data,
      } as any);
    } else {
      Inertia.post(`/next/admin/league/practices`, { practice: data } as any);
    }
  }
