...
export function LeaguePracticeForm() {
  ...
  const { form, bind } = useForm<Practice>(practice);

  const [state, setState] = useState({
    programId: practice.program_id,
    locationId: practice.location_id,
    seasonId: practice.season_id,
    divisionId: practice.division_id,
    awayTeamCustomName: practice.away_team_custom_name,
    homeTeamCustomName: practice.home_team_custom_name,
  });
  ...
  function handleFinish(data: Practice) {
    if (practice.id) {
      Inertia.patch(
        `/next/admin/league/practices/${practice.id}`,
        asPayload({ practice: data })
      );
    } else {
      Inertia.post(
        ...
        <Fieldset title="Logistics">
          <Space direction="vertical" style={{ width: "100%" }}>
            ...
            <Space>
              <Form.Item
                {...bind("starts_on")}
                initialValue={practice.starts_on && dayjs(practice.starts_on)}
                required
              >
                <DatePicker
                  showTime
