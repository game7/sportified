module StandingsHelper
  ...
  def link_to_standings(division)
    if division.current_season_id == season.id
      @path = season_standings_friendly_path(@division.slug)
    else
      @path = season_standings_friendly_path(@division.slug, season.slug)
    end
  ...
  def get_standings_season_url(division_slug, season_slug)
    standings_path(division_slug, season_slug)
  end
