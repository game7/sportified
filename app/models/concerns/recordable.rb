...
module Recordable
  ...
  def streak
    last_result + current_run.to_s if last_result
  end
  ...
  def calculate_record
    ...
    games.order(:starts_on).each do |game|
      ...
      self.longest_win_streak = self.current_run if result = 'W' && self.current_run > longest_win_streak
      self.longest_loss_streak = self.current_run if result = 'L' && self.current_run > longest_loss_streak
